...
RUN R --slave -e "BiocManager::install(c('BiocParallel','GenomicAlignments', 'GenomicRanges','rtracklayer', 'Rsamtools','limma','edgeR','org.Mm.eg.db','org.Hs.eg.db','org.Ce.eg.db','org.Dm.eg.db','ChIPseeker','clusterProfiler','Rsubread','SummarizedExperiment','DESeq2','APAlyzer', 'isomiRs','targetscan.Mm.eg.db','targetscan.Hs.eg.db'))"   
RUN mkdir -p /nl
RUN mkdir -p /project
RUN mkdir -p /share
